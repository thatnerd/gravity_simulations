...
def find_constant_accel(coords=None, accel_direction=np.array([0.0,-1.0]), 
		accel_strength=9.80, rotation_angle=-np.pi/2):
	"""
	Inputs
	------
	...
	accel_direction: if you want the force to point in another direction, put a
		unit vector here. Make sure that this is a unit vector with two 
		elements! (i.e., an np.array([x, y]) type object, with x and y being 
		numbers.) This will over
	accel_strength: (float) The magnitude of the acceleration vector.
	rotation_angle: (float): The angle that the force is, wrt the x-axis. 
	...
	if rotation_angle != -np.pi/2:
		if not np.equal(accel_direction, np.array([0, -1])).all:
			raise ConflictingInputError("You have passed both an " +
				"accel_direction and a rotation_angle to find_constant_accel.")
		accel_direction = np.dot(rotation_matrix_2d(rotation_angle), 
				np.array([1, 0]))
	if accel_strength < 0:
		raise NegativeMagnitudeError("Your accel_strength should be greater " +
				"than zero")
	if np.dot(accel_direction, accel_direction) != 1.0:
		raise ImproperUnitVectorLength("Your unit vector to represent the " + 
				"direction is not of length 1.")
...
def calculate_derivs(coords, accel=find_inverse_square_accel, *args, **kwargs):
	...
	derivs = np.array([[coords[:,1][0], coords[:,1][1]], 
		[accels[0], accels[1]]]).T
# [[xdot, xddot], [ydot, yddot]]
	return derivs
...
def projectile_step_time_simple(coords, dt, 
		accel_direction=np.array([0.0,-1.0]), accel_strength=9.80, 
...
def calculate_orbit(initial_coords, dt, strength=MGsun, 
		grav_origin=np.array([0.0, 0.0]), expected_period=None):
	"""
